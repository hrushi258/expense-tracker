...
export default function AddTransactionSheet({ open, onClose, prefillTransaction }) {
  const { categories, settings, triggerRefresh, selectedMonth } = useAppContext()
  ...
  const handleSubmit = async (e) => {
    ...
    const monthFromDate = `${new Date(form.date).getFullYear()}-${String(new Date(form.date).getMonth() + 1).padStart(2, '0')}`
  ...
  return (
    <BottomSheet
      open={open}
      onClose={onClose}
      title={prefillTransaction ? 'Edit Transaction' : 'Add Transaction'}
    >
          ...
          <div className="flex gap-2">
            ...
            {form.type === 'expense' && (
              <button
                ...
                className="px-3 py-2.5 rounded-xl bg-needs text-white text-xs font-semibold disabled:opacity-40 flex items-center gap-1.5 whitespace-nowrap transition-opacity"
                title="Auto-categorize with AI (your amounts are never sent)"
              >
                {aiLoading ? (
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {PILLARS.map(p => {
                  ...
                  const active = form.mainCategory === p
                  return (
                    <button
                      key={p}
                      type="button"
                      onClick={() => setForm(f => ({ ...f, mainCategory: p, subCategoryId: null }))}
                    ...
                    >
                      <span className="text-base">{['🏠','✨','🏦','📊'][PILLARS.indexOf(p)]}</span>
                      {meta.label}
                    </button>
                  )
                })}
