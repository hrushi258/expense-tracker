...
export default function Transactions() {
  const { selectedMonth, categories, pillarList, refreshTrigger } = useAppContext()
  ...
  useEffect(() => {
    setLoading(true)
    db.transactions
      .where('month').equals(selectedMonth).toArray()
    ...
    <div className="pb-4">
        ...
        <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1">
          {[{ key: ALL, label: 'All' }, { key: 'expense', label: 'Expense' }, { key: 'income', label: 'Income' }].map(f => (
            <button key={f.key} onClick={() => setFilterType(f.key)}
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${filterType === f.key ? 'bg-needs text-white border-needs' : 'bg-white text-slate-500 border-slate-200'}`}>
              {f.label}
            </button>
          ))}
          <div className="w-px bg-slate-200 flex-shrink-0" />
          {pillarList.map(p => (
            <button key={p.key} onClick={() => setFilterPillar(fp => fp === p.key ? ALL : p.key)}
              className="flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all"
              style={filterPillar === p.key
      ...
      <div className="px-4 mt-3 space-y-2">
        {loading ? (
          ...
          filtered.map(txn => <TransactionItem key={txn.id} txn={txn} categories={categories} />)
        )}
      </div>
    </div>
  )
