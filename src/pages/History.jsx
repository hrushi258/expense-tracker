...
import { getPreviousMonths, formatCurrency } from '../utils/formatters.js'
      ...
      .then(results => {
        setData(months.map((m, i) => {
          ...
          pillarList.forEach(p => {
            row[p.label] = txns.filter(t => t.type === 'expense' && t.mainCategory === p.key).reduce((s, t) => s + t.amount, 0)
          })
          return row
        }))
      })
        ...
        <div className="flex gap-2 mb-5">
          {[3, 6, 12].map(r => (
            <button key={r} onClick={() => setRange(r)}
              className={`px-4 py-2 rounded-full text-sm font-semibold border transition-all ${range === r ? 'bg-needs text-white border-needs' : 'bg-white text-slate-500 border-slate-200'}`}>
              {r}M
            </button>
          ))}
        </div>
            ...
            <div className="bg-white rounded-2xl shadow-card p-4 mb-4">
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={data}>
                  ...
                  <YAxis tickFormatter={v => v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v} tick={axisStyle} axisLine={false} tickLine={false} width={32} />
            ...
            <div className="bg-white rounded-2xl shadow-card p-4 mb-4">
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={data} barSize={12}>
                  ...
                  <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 12 }} />
                  {pillarList.map((p, i) => (
                    <Bar key={p.key} dataKey={p.label} stackId="a" fill={p.color}
                      radius={i === pillarList.length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0]} />
                  ))}
                </BarChart>
              </ResponsiveContainer>
            </div>
                    ...
                    {[...data].reverse().map(row => {
                      ...
                      return (
                        <tr key={row.month} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-3 py-3 text-right text-emerald-600 font-medium">{row.Income > 0 ? formatCurrency(row.Income) : '—'}</td>
                          <td className="px-3 py-3 text-right text-slate-700 font-medium">{row.Total > 0 ? formatCurrency(row.Total) : '—'}</td>
